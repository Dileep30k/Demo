...
namespace Edu.Repository.Repositories
{
    ...
    {
        public TemplateRepository(EduDbContext context)
            : base(context)
        { }
        ...
        public async Task<PagedList> GetTemplatePaged(Pagination pagination)
        {
            string searchText = IsPropertyExist(pagination.Filters, "searchText") ? pagination.Filters?.searchText : null;

            IRepository<TemplateModel> repository = new Repository<TemplateModel>(EduDbContext);
            var query = (from r in EduDbContext.Templates
                         where !r.IsDeleted
                         && (string.IsNullOrEmpty(searchText) || r.TemplateName.Contains(searchText))
