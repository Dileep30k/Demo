...
{
    public class BatchUserRepository : Repository<BatchUser>, IBatchUserRepository
    {
        public BatchUserRepository(EduDbContext context)
            : base(context)
        { }
        ...
        public async Task<PagedList> GetBatchUserPaged(Pagination pagination)
        {
            bool? isActive = IsPropertyExist(pagination.Filters, "isActive") ? pagination.Filters?.isActive : null;

            IRepository<BatchUserModel> repository = new Repository<BatchUserModel>(EduDbContext);
            var query = (from r in EduDbContext.BatchUsers
                         where !r.IsDeleted 
                         && (!isActive.HasValue || isActive.Value == r.IsActive)
                         select new BatchUserModel()
                         {
