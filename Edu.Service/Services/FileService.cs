...
namespace Edu.Service.Services
{
    ...
    public class FileService : PhysicalFileProvider, IFileService
    {
        ...
        {
            return Combine(WebRootPath, "wwwroot", path);
        }
        ...
        /// </summary>
        /// <param name="path">The path and name of the file to create</param>
        public virtual async Task<ResponseModel> CreateDocumentFile(Document entity)
        {
            ...
            try
            {
                ...
                entity.FilePath = Combine("Attachments", entity.DocumentType, entity.DocumentTableId.ToString());

                CreateDirectory(Combine(WebRootPath, "wwwroot", entity.FilePath));
                ...
                using (var stream = new FileStream(Combine(Directory.GetCurrentDirectory(), "wwwroot", entity.FilePath), FileMode.Create))
                {
                    await entity.DocumentFile.CopyToAsync(stream);
                }

                return new ResponseModel { Success = true, StatusCode = StatusCodes.Status200OK, Message = "Document created uploaded.", Data = entity };
            }
            catch (Exception)
            {
        ...
        public virtual ResponseModel CreateDocumentFileZipEntry(Document entity, string sourcePath)
        {
            try
            {
                entity.FilePath = Combine("Attachments", entity.DocumentType, entity.DocumentTableId.ToString());
                ...
                FileMove(Combine(sourcePath, entity.FileName), GetAbsolutePath(entity.FilePath));

                return new ResponseModel { Success = true, StatusCode = StatusCodes.Status200OK, Message = "Documents created.", Data = entity };
            }
            catch (Exception)
            {
